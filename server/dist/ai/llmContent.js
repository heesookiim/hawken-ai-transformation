 ...
 */
export function loadLLMContent(companyId) {
 ...
 */
export function saveLLMContent(companyId, content) {
    const filePath = getCachePath(companyId, 'llm_content');
 ...
 */
async function generateKeyBusinessChallenges(businessContext, industry) {
 ...
 */
async function generateStrategicOpportunities(businessContext, industry, keyBusinessChallenges) {
 ...
 */
async function generateExecutiveSummary(companyName, industry, businessContext, keyBusinessChallenges, strategicOpportunities) {
    try {
 ...
 */
export async function generateLLMContent(companyName, companyUrl, businessContext, industry, proposalData) {
    ...
    try {
        ...
        if (proposalData && proposalData.businessChallenges && proposalData.businessChallenges.length > 0) {
            ...
            keyBusinessChallenges = proposalData.businessChallenges.map((challenge) => {
            ...
            keyBusinessChallenges = proposalData.possiblePainPoints
                .slice(0, 5)
                .map((pp) => pp.description || pp.title || `Challenge in ${industry}`);
        }
        else {
        ...
        if (proposalData && proposalData.aiOpportunities && proposalData.aiOpportunities.length > 0) {
            ...
            strategicOpportunities = proposalData.aiOpportunities
                .slice(0, 5)
                .map((opp) => opp.title || opp.description || `AI opportunity for ${industry}`);
        }
        else {
        ...
        if (proposalData && proposalData.aiOpportunities && proposalData.aiOpportunities.length > 0) {
            ...
            if (proposalData.recommendedApproach) {
                const approachSentences = proposalData.recommendedApproach.split('.').map((s) => s.trim()).filter((s) => s.length > 0);
...
function formatChallengeDescription(challenge) {
...
function extractManifestations(challenge) {
    if (!challenge) {
...
function generateIndustryTerminology(industry) {
