...
app.use('/cache', (err, req, res, next) => {
    if (err.code === 'ENOENT') {
...
const getCacheDirectory = (companyId) => {
...
app.get('/api/analysis/:company', (req, res) => {
    try {
        ...
        if (data.aiOpportunities && Array.isArray(data.aiOpportunities)) {
            transformedData[`${company}-final-strategies-optimized.json`] = data.aiOpportunities.map((strategy) => {
...
function calculateAverage(items, scoreProp) {
    if (!items || items.length === 0)
        return 0;
    const sum = items.reduce((acc, item) => acc + (item[scoreProp] || 0), 0);
...
app.get('/api/llm-content/:company', async (req, res) => {
    try {
        ...
        const generatedContent = await generateLLMContent(proposalData.companyName, proposalData.companyUrl, proposalData.businessContext, proposalData.industry, proposalData // Pass the full proposal data to use existing content
        );
