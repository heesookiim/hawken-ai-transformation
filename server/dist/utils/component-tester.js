...
import { createStrategies } from '../ai/engines/strategy';
import { validateStrategies } from '../ai/engines/validation';
import { createImplementationPlan } from '../ai/engines/implementation';
import { checkFeasibility } from '../ai/engines/feasibility';
import fs from 'fs';
...
async function testIndividualComponents(companyName = "Sample Company", companyUrl = "https://example.com", description) {
    ...
    try {
        ...
        fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-scraped-data.json`), JSON.stringify(scrapedData, null, 2));
        ...
        fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-context.json`), JSON.stringify(contextResult, null, 2));
        ...
        fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-businessChallenges.json`), JSON.stringify(businessChallenges, null, 2));
        ...
        fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-industry.json`), JSON.stringify(industryResult, null, 2));
        ...
        let initialStrategies = await createStrategies(businessChallenges, industryResult.industryInsights);
        fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-strategies-initial.json`), JSON.stringify(initialStrategies, null, 2));
        console.log("   Initial strategy creation completed");
        // Initial validation
        let validationResult = await validateStrategies(initialStrategies, contextResult.businessContext, industryResult.industry);
        fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-validation-initial.json`), JSON.stringify(validationResult, null, 2));
        ...
        for (let iteration = 0; iteration < MAX_VALIDATION_ITERATIONS - 1; iteration++) {
            ...
            const refinedStrategies = await createStrategies(businessChallenges, industryResult.industryInsights, feedback);
            ...
            fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-strategies-iteration-${iteration + 1}.json`), JSON.stringify(nextStrategies, null, 2));
            // Validate the new set of strategies
            validationResult = await validateStrategies(nextStrategies, contextResult.businessContext, industryResult.industry);
            fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-validation-iteration-${iteration + 1}.json`), JSON.stringify(validationResult, null, 2));
        ...
        const finalValidatedStrategies = optimizedStrategies.filter(s => s.validationScore >= MIN_STRATEGY_SCORE_REQUIRED);
        ...
        fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-strategies-optimized.json`), JSON.stringify(finalValidatedStrategies, null, 2));
        ...
        fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-implementation.json`), JSON.stringify(implementationStrategies, null, 2));
        ...
        fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-feasibility-initial.json`), JSON.stringify(feasibilityResult, null, 2));
        ...
        while (iteration < MAX_FEASIBILITY_ITERATIONS - 1 &&
            ...
            fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-implementation-iteration-${iteration}.json`), JSON.stringify(refinedImplementationStrategies, null, 2));
            // Check feasibility of the refined implementation
            feasibilityResult = await checkFeasibility(refinedImplementationStrategies);
            fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-feasibility-iteration-${iteration}.json`), JSON.stringify(feasibilityResult, null, 2));
            ...
            implementationStrategies.forEach((strategy, index) => {
                ...
                const evolution = feasibilityEvolutions[index];
                evolution.iterations.push({
                    strategy: refinedImplementationStrategies[index],
                    feasibilityScore: feasibilityData.feasibilityScore,
                    feedback: feasibilityData
                });
        ...
        fs.writeFileSync(path.join(outputDir, `${companyName.toLowerCase()}-final-strategies-optimized.json`), JSON.stringify(finalStrategiesWithScores, null, 2));
...
testIndividualComponents(company.name, company.url, company.description)
    ...
    fs.writeFileSync(path.join(outputDir, `${company.name.toLowerCase()}-complete-results.json`), JSON.stringify(results, null, 2));
    // Save just the final report for easier analysis
    fs.writeFileSync(path.join(outputDir, `${company.name.toLowerCase()}-final-report.json`), JSON.stringify(results.finalReport, null, 2));
