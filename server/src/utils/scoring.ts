import { PainPointRelevance } from '../ai/engines/validation.js';
import { BusinessChallengeRelevance } from '../ai/engines/validation.js';

/**
 * Interface for any strategy object with the minimum required properties for scoring
 */
 ...
 */
export function calculateOpportunityScore(strategy: ScoringStrategy): number {
  ...
  if (strategy.painPointRelevances && strategy.painPointRelevances.length > 0) {
    ...
    painPointRelevanceScore = (topRelevances.reduce((sum: number, rel: PainPointRelevance) => sum + rel.relevanceScore, 0) / topRelevances.length) * 10;
  }
  ...
  if (strategy.businessChallengeRelevances && strategy.businessChallengeRelevances.length > 0) {
    ...
    // Average their scores (0-10 scale) and convert to 0-100 scale
    businessChallengeRelevanceScore = (topRelevances.reduce((sum: number, rel: BusinessChallengeRelevance) => sum + rel.relevanceScore, 0) / topRelevances.length) * 10;
  }
 ...
 * @returns A score between 0-100
 */
export function calculateCombinedScore(strategy: ScoringStrategy): number {
