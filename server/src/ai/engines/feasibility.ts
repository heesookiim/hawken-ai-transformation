...
interface FeasibilityAnalysis extends Strategy {
  ...
  feasibilityCriteria: {
    technicalFeasibility: number;   // 0-25 points
    resourceRequirements: number;   // 0-25 points (lower score = more resources needed)
    riskAssessment: number;         // 0-25 points (higher = lower risk)
    implementationComplexity: number; // 0-25 points (higher = less complex)
  };
...
export async function checkFeasibility(
  ...
  try {
    ...
    feasibilityAnalysis.forEach((strategy: FeasibilityAnalysis, index: number) => {
    ...
    const averageScore = feasibilityAnalysis.reduce((sum: number, s: FeasibilityAnalysis) => 
      sum + s.feasibilityScore, 0) / feasibilityAnalysis.length;
