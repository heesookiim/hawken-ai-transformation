...
import { ValidatedStrategy } from './validation.js';
...
export interface StrategyFeedback {
  strategies: ValidatedStrategy[];  // Strategies that need improvement
  feedbackComments: string[];       // General feedback on strategy approach
  industryContext: string;          // Additional industry context
}
 ...
 */
export function validateStrategiesBeforeImplementation(
  strategies: Strategy[],
  options: {
    logWarnings?: boolean;
    enrichWithWarnings?: boolean;
  } = { logWarnings: true, enrichWithWarnings: true }
): {
  strategies: Strategy[];
  warnings: { strategyId: string; message: string; severity: 'high' | 'medium' | 'low' }[];
  allValid: boolean;
} {
  const warnings: { strategyId: string; message: string; severity: 'high' | 'medium' | 'low' }[] = [];
...
export async function createStrategies(
  ...
  try {
    ...
    return jsonResponse.strategies.map((strategy: any, index: number) => ({
      ...strategy,
