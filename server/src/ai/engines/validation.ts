...
import { PossiblePainPoint } from './industry.js';
...
export interface ValidatedStrategy extends Strategy {
  ...
  validationCriteria: {
    ...
    marketPotential: number;    // 0-25 points
    industryRelevance: number;  // 0-25 points 
    clarity: number;            // 0-25 points, measures LLM suitability and clarity
  };
...
async function assessPainPointRelevance(
  strategies: Strategy[],
  painPoints: PossiblePainPoint[]
): Promise<Record<string, PainPointRelevance[]>> {
  ...
  try {
    ...
    const transformedRelevances: Record<string, PainPointRelevance[]> = {};
    
    // For each strategy
    Object.entries(relevanceMap).forEach(([strategyId, painPointMap]: [string, any]) => {
      transformedRelevances[strategyId] = [];
      
      // For each pain point relevance
      Object.entries(painPointMap).forEach(([painPointId, relevance]: [string, any]) => {
...
async function assessBusinessChallengeRelevance(
  strategies: Strategy[],
  businessChallenges: any[]
): Promise<Record<string, BusinessChallengeRelevance[]>> {
  ...
  try {
    ...
    // Transform into our expected format
    const transformedRelevances: Record<string, BusinessChallengeRelevance[]> = {};
    
    // For each strategy
    Object.entries(relevanceMap).forEach(([strategyId, challengeMap]: [string, any]) => {
      transformedRelevances[strategyId] = [];
      
      // For each business challenge relevance
      Object.entries(challengeMap).forEach(([challengeId, relevance]: [string, any]) => {
        // Ensure score is within 0-10 range
        const relevanceScore = Math.min(10, Math.max(0, relevance.relevanceScore || 0));
...
export async function validateStrategies(
  ...
  try {
    ...
    const validatedStrategies: ValidatedStrategy[] = jsonResponse.validatedStrategies;
    
    // Add pain point relevance data to each strategy
    validatedStrategies.forEach((strategy: ValidatedStrategy) => {
      ...
      if (
        strategy.validationCriteria.businessAlignment &&
    ...
    const averageScore = validatedStrategies.reduce(
      (sum: number, s: ValidatedStrategy) => sum + s.validationScore, 
      0
