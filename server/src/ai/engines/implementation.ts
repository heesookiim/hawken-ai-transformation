...
import { PainPointRelevance, BusinessChallengeRelevance } from './validation.js';
...
// Configuration options for the implementation planning process
export interface ImplementationPlanOptions {
  failOnError?: boolean;                // Whether to throw on parse errors
  requireMinimumStrategies?: boolean;   // Whether to require a minimum number of strategies
  minimumStrategies?: number;           // Minimum number of valid strategies required (default: 1)
  allowPartialSuccess?: boolean;        // Whether to return only successfully parsed strategies
  validateLLMFeasibility?: boolean;     // Whether to validate if strategies are feasible with LLMs
  fallbackToGeneric?: boolean;          // Whether to use generic fallbacks for failed strategies
}

// Default options
const DEFAULT_OPTIONS: ImplementationPlanOptions = {
  failOnError: false,
...
function createGenericImplementationPlan(strategy: Strategy): ImplementationStrategy {
  return {
    ...strategy,
    keyBenefits: [
...
function validateLLMFeasibility(strategy: Strategy): boolean {
...
export async function createImplementationPlan(
  strategies: Strategy[],
  options: Partial<ImplementationPlanOptions> = {}
  ...
  if (config.requireMinimumStrategies && 
    ...
    const error = new NoValidStrategiesError(
      `Not enough valid strategies. Required: ${config.minimumStrategies}, Found: ${validStrategies.length}`
    );
    if (config.failOnError) {
      throw error;
    }
  ...
  try {
    ...
    try {
      ...
      const processedPlans: ImplementationStrategy[] = [];
      ...
      if (config.requireMinimumStrategies && 
        ...
        if (config.failOnError) {
          throw new ImplementationPlanError(
            `Not enough valid plans. Required: ${config.minimumStrategies}, Found: ${processedPlans.length}`
          );
        }
    ...
    // Throw error if configured to fail on error
    if (config.failOnError) {
      throw new ImplementationPlanError('AI model call failed', callError as Error);
    }
