import { scrapeCompanyWebsite, ScrapedData } from '../scraper/index.js';
import { analyzeContext } from './engines/context.js';
import { generateBusinessChallenges } from './engines/businessChallenges.js';
import { getIndustryInsights, IndustryResult, PossiblePainPoint } from './engines/industry.js';
import { StrategyFeedback, Strategy, createStrategies as generateStrategies, validateStrategiesBeforeImplementation } from './engines/strategy.js';
import { validateStrategies, ValidationResult, ValidatedStrategy, PainPointRelevance, BusinessChallengeRelevance } from './engines/validation.js';
import { createImplementationPlan } from './engines/implementation.js';
import { checkFeasibility, FeasibilityResult } from './engines/feasibility.js';
...
const writeCache = (companyId: string, step: string, data: any) => {
...
const readCache = (companyId: string, step: string) => {
  const filePath = getCachePath(companyId, step);
...
function generateMockData(companyName: string, companyUrl: string) {
  return {
...
// Interface for implementation strategy
...
// Interface for feasibility analysis
...
// Type for cached iteration data
interface CachedFeasibilityIteration {
  refinedStrategies: ImplementationStrategy[];
 ...
 */
export function loadProposal(companyName: string, companyUrl: string): ProposalData | null {
...
export async function generateProposal(companyUrl: string, companyName: string) {
  try {
    const companyId = getCompanyId(companyName);
    ...
    if (cachedStrategies) {
      ...
      aiStrategies = await generateStrategies(
        businessChallenges, 
    ...
    if (cachedOptimizedStrategies) {
      ...
      let validationResult = await validateStrategies(
        ...
        industryResult.possiblePainPoints,
        businessChallenges
      );
      ...
      if (validationResult.requiresFeedback) {
        ...
        validationResult = await optimizeWithFeedback(
          validationResult,
          businessChallenges,
          industryResult.industryInsights
        );
    ...
    const { strategies: enrichedStrategies, warnings, allValid } = validateStrategiesBeforeImplementation(
      finalValidatedStrategies,
      { 
        logWarnings: true,
    ...
    if (warnings.length > 0) {
      ...
      const severityCounts = warnings.reduce((counts: Record<string, number>, w: { severity: string }) => {
    ...
    if (cachedFinalImplementations) {
      ...
      const feasibilityEvolutions: FeasibilityEvolution[] = implementationStrategies.map((strategy: ImplementationStrategy) => ({
        id: strategy.id,
      ...
      implementationStrategies.forEach((strategy: ImplementationStrategy, index: number) => {
      ...
      while (iteration < MAX_FEASIBILITY_ITERATIONS - 1 && 
        ...
        const cachedIteration: CachedFeasibilityIteration | null = readCache(companyId, `feasibility_iteration_${iteration + 1}`);
        if (cachedIteration) {
          console.log(`   Loading feasibility iteration ${iteration + 1} from cache`);
          const feasibilityResult: FeasibilityResult = cachedIteration.feasibilityResult;
          const refinedImplementationStrategies: ImplementationStrategy[] = cachedIteration.refinedStrategies;
          
          // Record this iteration's results
          refinedImplementationStrategies.forEach((strategy: ImplementationStrategy, index: number) => {
            const feasibilityData = feasibilityResult.strategies[index];
            if (!feasibilityData) return;
          ...
          strategiesNeedingImprovement = feasibilityResult.strategies.filter(
            (s: FeasibilityAnalysis) => s.feasibilityScore < FEASIBILITY_SCORE_THRESHOLD
          );
        ...
        const refinedImplementationStrategies = implementationStrategies.map((strategy: ImplementationStrategy, index: number) => {
          const feasibilityData = feasibilityResult.strategies[index];
      ...
      finalStrategiesWithScores = optimizedImplementations.map((strategy: ImplementationStrategy) => ({
        ...strategy,
    ...
    const proposalData: ProposalData = {
      ...
      aiOpportunities: finalStrategiesWithScores.map((strategy: ImplementationStrategy) => ({
        id: strategy.id,
 ...
 */
function triggerLLMContentGeneration(companyName: string, companyUrl: string, proposalData: ProposalData) {
  const companyId = getCompanyId(companyName);
  ...
  (async () => {
    try {
      await generateLLMContent(
        ...
        proposalData  // Pass the full proposal data to avoid LLM API calls
      );
...
function getStrategyValidationAndFeasibility(s: Strategy): Promise<Strategy> {
...
async function validateStrategy(strategy: Strategy): Promise<Strategy> {
...
function addScoreAndId(strategy: Strategy, index: number): Strategy {
    return {
    ...strategy,
...
export function determineStrategyCategory(strategy: Strategy): string {
...
export async function createStrategies(
  ...
  try {
    ...
    let strategies: Strategy[] = JSON.parse(cleanedJson);
...
async function optimizeWithFeedback(
  ...
  for (let iteration = 0; iteration < MAX_VALIDATION_ITERATIONS - 1; iteration++) {
    ...
    const strategiesNeedingImprovement = validationResult.strategies.filter(
      (s: ValidatedStrategy) => s.validationScore < STRATEGY_INDIVIDUAL_THRESHOLD
    );
    ...
    const refinedStrategies = await generateStrategies(
      businessChallenges,
      industryInsights,
      feedback
    );
    ...
    const nextStrategies = validationResult.strategies.map(strategy => {
      if (strategy.validationScore >= STRATEGY_INDIVIDUAL_THRESHOLD) {
        ...
        } as ValidatedStrategy : strategy;
      }
    });
    
    // Validate the new set of strategies
    validationResult = await validateStrategies(
      nextStrategies as Strategy[], // Cast to expected type for validation function
  ...
  const finalStrategies = optimizedStrategies.filter(
    (s: ValidatedStrategy) => s.validationScore >= MIN_STRATEGY_SCORE_REQUIRED
  );
  
  // Calculate the new average score
  const newAverageScore = finalStrategies.reduce(
    (sum, s) => sum + s.validationScore, 
    0
